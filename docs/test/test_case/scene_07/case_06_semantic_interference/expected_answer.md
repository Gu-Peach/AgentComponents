# Expected Answer: scene_07_case_06
...
|---|---|---|---|---|
| `input_feed` | `continuous` | `source_station_1`, `input_conveyor_1` | `runtime.sim_start` | `input_conveyor_1.part_ready` |
| `machine_processing` | `sequential` | `robot_1` | `input_conveyor_1.part_ready` | `machine.processing_done` |
| `output_release` | `continuous` | `robot_1`, `output_conveyor_1` | `machine.processing_done` | `output_conveyor_1.material_released` |
...
| `input_conveyor_1.part_ready` | `device_signal` | input_conveyor_1.part_ready 场景事件 |
| `machine.processing_start` | `device_signal` | machine.processing_start 场景事件 |
| `machine.processing_done` | `device_signal` | machine.processing_done 场景事件 |
...
|---|---|---|---|---|
| `accept_material_to_first_available_stop_point` | `machine_processing` | `event` | `start_behavior` | 新物料进入传送带时写入入口侧最近可用停留点。 |
| `advance_material_to_next_stop_point` | `machine_processing` | `event` | `start_behavior` | 前方停留点可用时，物料沿出口方向推进一个停留点。 |
| `wait_when_next_stop_point_occupied` | `machine_processing` | `event` | `update_state` | 前方停留点或下游不可用时，将物料留在当前位置并进入等待队列。 |
| `release_material_when_downstream_available` | `machine_processing` | `event` | `start_behavior` | 出口停留点有物料且下游可接收时释放物料。 |
| `emit_blocked_when_no_stop_point_available` | `machine_processing` | `state_change` | `emit_event` | 无可用停留点或达到容量上限时发出阻塞观测。 |
| `emit_capacity_available_when_stop_point_released` | `machine_processing` | `event` | `emit_event` | 停留点释放且负载低于恢复阈值时发出容量恢复事件。 |
| `source_feeds_input_conveyor` | `input_feed` | `event` | `start_behavior` | 输入来源持续将物料送到输入传送带。 |
| `robot_loads_machine_buffer` | `machine_processing` | `event` | `start_behavior` | 机械臂从输入传送带取料到固定加工位。 |
| `processing_timer_completes` | `machine_processing` | `state_change` | `emit_event` | 加工计时完成后产生离散完成事件。 |
| `robot_unloads_processed_part` | `output_release` | `event` | `start_behavior` | 机械臂将加工完成物料放到输出传送带。 |
...
| `target_conveyor_selection` | `load_balancing` | {"candidates": ["input_conveyor_1", "output_conveyor_1"], "prefer": "lowest_current_load_not_blocked"} |

### 1.6 期望完成条件 `completion_conditions`

| condition_id | 条件表达 | 依赖状态 |
|---|---|---|
| completion_01 | `processed_parts.count == requested_count` | `state_model` / RuntimeSnapshot |
| completion_02 | `machine_process_buffer.occupied_by == null` | `state_model` / RuntimeSnapshot |
