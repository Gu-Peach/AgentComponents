# Expected Answer: scene_06_case_06

## 1. 干扰信息说明

| 干扰类型 | 内容 | 期望处理 |
|---|---|---|
| 旧 schema 干扰 | 用户提到旧方案名称。 | 忽略旧方案名称，最终仍输出当前 `SceneBehaviorGraph`。 |
| 模糊表达 | 用户说“谁空谁拿”“别堵住”。 | 转换为 `resource_lock`、`shared_pool_claim`、`queue_wait` 和 `capacity_threshold`。 |
| 无关描述 | 非运行目标称呼。 | 不进入核心行为图。 |
...
|---|---|---|---|---|
| `inbound_feed` | `continuous` | `source_station_1`, `inbound_conveyor_1` | `runtime.sim_start` | `inbound_conveyor_1.part_ready` |
| `store_with_lift` | `sequential` | `inbound_lift_1`, `storage_rack_1` | `inbound_conveyor_1.part_ready` | `storage_rack_1.stored` |
| `retrieve_with_lift` | `sequential` | `storage_rack_1`, `outbound_lift_1` | `storage.release_requested` | `outbound_lift_1.material_ready` |
| `outbound_transport` | `continuous` | `outbound_conveyor_1` | `outbound_lift_1.material_ready` | `outbound_conveyor_1.material_released` |
...
| `inbound_conveyor_1.part_ready` | `device_signal` | inbound_conveyor_1.part_ready 场景事件 |
| `storage.cell_reserved` | `device_signal` | storage.cell_reserved 场景事件 |
| `storage_rack_1.stored` | `device_signal` | storage_rack_1.stored 场景事件 |
| `storage.release_requested` | `device_signal` | storage.release_requested 场景事件 |
| `outbound_lift_1.material_ready` | `device_signal` | outbound_lift_1.material_ready 场景事件 |
| `global.storage_cycle_done` | `global_event` | global.storage_cycle_done 场景事件 |
...
|---|---|---|---|---|
| `accept_material_to_first_available_stop_point` | `store_with_lift` | `event` | `start_behavior` | 新物料进入传送带时写入入口侧最近可用停留点。 |
| `advance_material_to_next_stop_point` | `store_with_lift` | `event` | `start_behavior` | 前方停留点可用时，物料沿出口方向推进一个停留点。 |
| `wait_when_next_stop_point_occupied` | `store_with_lift` | `event` | `update_state` | 前方停留点或下游不可用时，将物料留在当前位置并进入等待队列。 |
| `release_material_when_downstream_available` | `store_with_lift` | `event` | `start_behavior` | 出口停留点有物料且下游可接收时释放物料。 |
| `emit_blocked_when_no_stop_point_available` | `store_with_lift` | `state_change` | `emit_event` | 无可用停留点或达到容量上限时发出阻塞观测。 |
| `emit_capacity_available_when_stop_point_released` | `store_with_lift` | `event` | `emit_event` | 停留点释放且负载低于恢复阈值时发出容量恢复事件。 |
| `source_feeds_inbound_conveyor` | `inbound_feed` | `event` | `start_behavior` | 入口来源将物料送入入库传送带。 |
| `inbound_lift_accepts_material` | `store_with_lift` | `event` | `start_behavior` | 入口升降台从入库传送带接收物料。 |
| `reserve_storage_cell` | `store_with_lift` | `event` | `start_behavior` | 入库前先 reserve 空闲库位。 |
| `store_material_to_reserved_cell` | `store_with_lift` | `event` | `start_behavior` | 升降台移动到目标层后将物料放入已预留库位。 |
| `retrieve_fifo_cell` | `retrieve_with_lift` | `event` | `start_behavior` | 出库按 FIFO 选择已存储库位释放。 |
| `outbound_lift_releases_to_conveyor` | `outbound_transport` | `event` | `start_behavior` | 出库升降台将物料交给末端传送带。 |
...
| `target_conveyor_selection` | `load_balancing` | {"candidates": ["inbound_conveyor_1", "outbound_conveyor_1"], "prefer": "lowest_current_load_not_blocked"} |
| `storage_slot_selection` | `deterministic_priority` | {"strategy": "first_available_cell"} |
| `storage_release_selection` | `deterministic_priority` | {"strategy": "fifo_by_store_time"} |

### 1.6 期望完成条件 `completion_conditions`

| condition_id | 条件表达 | 依赖状态 |
|---|---|---|
| completion_01 | `requested inbound batch stored and requested outbound batch released` | `state_model` / RuntimeSnapshot |
| completion_02 | `all lift platforms empty` | `state_model` / RuntimeSnapshot |
