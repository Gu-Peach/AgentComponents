# Expected Answer: scene_02_case_06
...
|---|---|---|---|---|
| `pallet_feed` | `continuous` | `pallet_source_1`, `main_conveyor_1` | `runtime.sim_start` | `main_conveyor_1.station_candidate_ready` |
| `mainline_dispatch` | `parallel_continuous` | `main_conveyor_1`, `robot_1`, `robot_2`, `robot_3` | `main_conveyor_1.station_candidate_ready` | `global.pallet_batch_sorted` |
| `robot_output_conveying` | `continuous` | `robot_1_out_conveyor`, `robot_2_out_conveyor`, `robot_3_out_conveyor` | `robot.pick_done` | `output_conveyor.material_released` |
...
| `pallet_source.material_ready` | `device_signal` | pallet_source.material_ready 场景事件 |
| `main_conveyor_1.station_candidate_ready` | `device_signal` | main_conveyor_1.station_candidate_ready 场景事件 |
| `mainline.pause_feed` | `device_signal` | mainline.pause_feed 场景事件 |
| `mainline.resume_feed` | `device_signal` | mainline.resume_feed 场景事件 |
| `global.pallet_batch_sorted` | `global_event` | global.pallet_batch_sorted 场景事件 |
...
|---|---|---|---|---|
| `accept_material_to_first_available_stop_point` | `mainline_dispatch` | `event` | `start_behavior` | 新物料进入传送带时写入入口侧最近可用停留点。 |
| `advance_material_to_next_stop_point` | `mainline_dispatch` | `event` | `start_behavior` | 前方停留点可用时，物料沿出口方向推进一个停留点。 |
| `wait_when_next_stop_point_occupied` | `mainline_dispatch` | `event` | `update_state` | 前方停留点或下游不可用时，将物料留在当前位置并进入等待队列。 |
| `release_material_when_downstream_available` | `mainline_dispatch` | `event` | `start_behavior` | 出口停留点有物料且下游可接收时释放物料。 |
| `emit_blocked_when_no_stop_point_available` | `mainline_dispatch` | `state_change` | `emit_event` | 无可用停留点或达到容量上限时发出阻塞观测。 |
| `emit_capacity_available_when_stop_point_released` | `mainline_dispatch` | `event` | `emit_event` | 停留点释放且负载低于恢复阈值时发出容量恢复事件。 |
| `produce_next_pallet` | `pallet_feed` | `event` | `start_behavior` | 托盘来源按节拍生成新托盘。 |
| `move_pallet_to_farthest_idle_station` | `mainline_dispatch` | `event` | `start_behavior` | 主线优先把新托盘送到最远端空闲机械臂工位。 |
| `assigned_robot_sorts_pallet` | `mainline_dispatch` | `event` | `start_behavior` | 已分配机械臂处理对应工位托盘。 |
| `pause_mainline_when_all_robots_busy` | `pallet_feed` | `state_change` | `emit_event` | 三台机械臂全忙时暂停主线和出料口。 |
...
| `target_conveyor_selection` | `load_balancing` | {"candidates": ["main_conveyor_1", "robot_1_out_conveyor", "robot_2_out_conveyor", "robot_3_out_conveyor"], "prefer": "lowest_current_load_not_blocked"} |

### 1.6 期望完成条件 `completion_conditions`

| condition_id | 条件表达 | 依赖状态 |
|---|---|---|
| completion_01 | `pallet_queue.empty == true` | `state_model` / RuntimeSnapshot |
| completion_02 | `all assigned pallet workpiece pools empty` | `state_model` / RuntimeSnapshot |
| completion_03 | `active_actions.empty == true` | `state_model` / RuntimeSnapshot |
| completion_04 | `all conveyor_occupancy stop points are empty` | `state_model` / RuntimeSnapshot |
