# Expected Answer: scene_05_case_06
...
|---|---|---|---|---|
| `round_table_feed` | `continuous` | `round_table_1` | `runtime.sim_start` | `round_table.material_available` |
| `dual_robot_dispatch` | `parallel_continuous` | `robot_1`, `robot_2` | `round_table.material_available` | `global.dispatch_done` |
| `output_conveyor_dispatch` | `continuous` | `out_conveyor_1`, `out_conveyor_2`, `out_conveyor_3`, `out_conveyor_4` | `robot.pick_done` | `output_conveyor.material_released` |
...
| `round_table.material_available` | `device_signal` | round_table.material_available 场景事件 |
| `output_conveyor.material_arrived` | `device_signal` | output_conveyor.material_arrived 场景事件 |
| `global.dispatch_done` | `global_event` | global.dispatch_done 场景事件 |
...
|---|---|---|---|---|
| `accept_material_to_first_available_stop_point` | `dual_robot_dispatch` | `event` | `start_behavior` | 新物料进入传送带时写入入口侧最近可用停留点。 |
| `advance_material_to_next_stop_point` | `dual_robot_dispatch` | `event` | `start_behavior` | 前方停留点可用时，物料沿出口方向推进一个停留点。 |
| `wait_when_next_stop_point_occupied` | `dual_robot_dispatch` | `event` | `update_state` | 前方停留点或下游不可用时，将物料留在当前位置并进入等待队列。 |
| `release_material_when_downstream_available` | `dual_robot_dispatch` | `event` | `start_behavior` | 出口停留点有物料且下游可接收时释放物料。 |
| `emit_blocked_when_no_stop_point_available` | `dual_robot_dispatch` | `state_change` | `emit_event` | 无可用停留点或达到容量上限时发出阻塞观测。 |
| `emit_capacity_available_when_stop_point_released` | `dual_robot_dispatch` | `event` | `emit_event` | 停留点释放且负载低于恢复阈值时发出容量恢复事件。 |
| `present_round_table_material` | `round_table_feed` | `event` | `start_behavior` | 圆桌固定站点呈现待处理物料。 |
| `idle_robot_claims_round_table_part` | `dual_robot_dispatch` | `event` | `emit_event` | 两台机械臂谁空谁 claim 圆桌上的一个物料。 |
| `claimed_part_to_least_loaded_conveyor` | `dual_robot_dispatch` | `event` | `start_behavior` | claim 成功后选择未 blocked 且负载较低的出料传送带。 |
...
| `target_conveyor_selection` | `load_balancing` | {"candidates": ["out_conveyor_1", "out_conveyor_2", "out_conveyor_3", "out_conveyor_4"], "prefer": "lowest_current_load_not_blocked"} |
...
|---|---|---|
| completion_01 | `round_table_pool.station_a.empty == true` | `state_model` / RuntimeSnapshot |
| completion_02 | `all conveyor_loads current_load == 0` | `state_model` / RuntimeSnapshot |
