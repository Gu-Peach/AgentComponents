# scene_08_case_06_semantic_interference

## 1. Case 元信息

| 字段 | 内容 |
|---|---|
| case_id | scene_08_case_06 |
| case_type | semantic_interference |
| source_scene | scene_08 |
...
| 字段 | 内容 |
|---|---|
| raw_description_summary_source | `docs/test/case/scene_08/normalized_case.md#raw_description_summary` |
| scene_image | `docs/business/test/8.png` |
| raw_description_summary | 物料侧出料台和传送带持续送料，托盘侧传送带同步运输空托盘；当物料和托盘都到位时，机械臂把物料装到托盘，达到数量后托盘输出。 |
| case_user_goal | 按这个场景生成图就行，但我以前叫它 SimPlan，也可能说成 SignalBusSchema；别被名字带偏。实际目标是图里的物料与托盘同步到位装载顺畅运行，谁空谁拿、别堵住、满了就等。 |
