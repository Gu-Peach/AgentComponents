# scene_04_case_06_semantic_interference

## 1. Case 元信息

| 字段 | 内容 |
|---|---|
| case_id | scene_04_case_06 |
| case_type | semantic_interference |
| source_scene | scene_04 |
| based_on_case | scene_04_case_01 |
| difficulty | high |
...
| 字段 | 内容 |
|---|---|
| raw_description_summary_source | `docs/test/case/scene_04/normalized_case.md#raw_description_summary` |
| scene_image | `docs/business/test/4.png` |
| raw_description_summary | 传送带起点直接生成或接收物料，物料移动到中间加工停留点后由机械臂执行固定位置操作，完成后继续沿传送带输出。 |
| case_user_goal | 按这个场景生成图就行，但我以前叫它 SimPlan，也可能说成 SignalBusSchema；别被名字带偏。实际目标是图里的传送带中段机械臂加工模拟顺畅运行，谁空谁拿、别堵住、满了就等。 |
