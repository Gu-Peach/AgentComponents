# Expected Answer: scene_04_case_06
...
|---|---|---|---|---|
| `inline_feed` | `continuous` | `source_station_1`, `main_conveyor_1` | `runtime.sim_start` | `main_conveyor_1.processing_stop_point_ready` |
| `inline_processing` | `sequential` | `robot_1`, `robot_2` | `main_conveyor_1.processing_stop_point_ready` | `processing.done` |
| `post_process_release` | `continuous` | `main_conveyor_1` | `processing.done` | `main_conveyor_1.material_released` |
...
| `main_conveyor_1.processing_stop_point_ready` | `device_signal` | main_conveyor_1.processing_stop_point_ready 场景事件 |
| `processing.start` | `device_signal` | processing.start 场景事件 |
| `processing.done` | `device_signal` | processing.done 场景事件 |
| `global.processing_batch_done` | `global_event` | global.processing_batch_done 场景事件 |
...
|---|---|---|---|---|
| `accept_material_to_first_available_stop_point` | `inline_processing` | `event` | `start_behavior` | 新物料进入传送带时写入入口侧最近可用停留点。 |
| `advance_material_to_next_stop_point` | `inline_processing` | `event` | `start_behavior` | 前方停留点可用时，物料沿出口方向推进一个停留点。 |
| `wait_when_next_stop_point_occupied` | `inline_processing` | `event` | `update_state` | 前方停留点或下游不可用时，将物料留在当前位置并进入等待队列。 |
| `release_material_when_downstream_available` | `inline_processing` | `event` | `start_behavior` | 出口停留点有物料且下游可接收时释放物料。 |
| `emit_blocked_when_no_stop_point_available` | `inline_processing` | `state_change` | `emit_event` | 无可用停留点或达到容量上限时发出阻塞观测。 |
| `emit_capacity_available_when_stop_point_released` | `inline_processing` | `event` | `emit_event` | 停留点释放且负载低于恢复阈值时发出容量恢复事件。 |
| `feed_material_to_processing_stop_point` | `inline_feed` | `event` | `start_behavior` | 上料台生成物料并送入主传送带。 |
| `hold_part_at_processing_stop_point` | `inline_processing` | `event` | `update_state` | 物料到达中间加工停留点后暂停继续前进。 |
| `robot_processes_fixed_position` | `inline_processing` | `event` | `start_behavior` | 空闲机械臂在固定位置执行一次模拟加工。 |
| `release_processed_part` | `post_process_release` | `event` | `start_behavior` | 加工完成后释放停留点并允许继续输送。 |
...
| completion_02 | `processing_stop_point.occupied_by == null` | `state_model` / RuntimeSnapshot |
