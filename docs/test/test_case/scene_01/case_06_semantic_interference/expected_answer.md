# Expected Answer: scene_01_case_06
...
|---|---|---|---|---|
| `pallet_transport` | `sequential` | `pallet_1`, `main_conveyor_1`, `main_conveyor_2` | `runtime.sim_start` | `main_conveyor_2.pallet_ready` |
| `parallel_robot_sorting` | `parallel_continuous` | `robot_1`, `robot_2` | `main_conveyor_2.pallet_ready` | `global.sorting_done` |
| `output_conveying` | `continuous` | `upper_out_conveyor_1`, `lower_out_conveyor_1` | `robot.pick_done` | `output_conveyor.material_released` |
...
| `main_conveyor_2.pallet_ready` | `device_signal` | main_conveyor_2.pallet_ready 场景事件 |
| `output_conveyor.material_arrived` | `device_signal` | output_conveyor.material_arrived 场景事件 |
| `robot.pause_pick` | `device_signal` | robot.pause_pick 场景事件 |
| `robot.resume_pick` | `device_signal` | robot.resume_pick 场景事件 |
| `global.sorting_done` | `global_event` | global.sorting_done 场景事件 |
...
|---|---|---|---|---|
| `accept_material_to_first_available_stop_point` | `pallet_transport` | `event` | `start_behavior` | 新物料进入传送带时写入入口侧最近可用停留点。 |
| `advance_material_to_next_stop_point` | `pallet_transport` | `event` | `start_behavior` | 前方停留点可用时，物料沿出口方向推进一个停留点。 |
| `wait_when_next_stop_point_occupied` | `pallet_transport` | `event` | `update_state` | 前方停留点或下游不可用时，将物料留在当前位置并进入等待队列。 |
| `release_material_when_downstream_available` | `pallet_transport` | `event` | `start_behavior` | 出口停留点有物料且下游可接收时释放物料。 |
| `emit_blocked_when_no_stop_point_available` | `pallet_transport` | `state_change` | `emit_event` | 无可用停留点或达到容量上限时发出阻塞观测。 |
| `emit_capacity_available_when_stop_point_released` | `pallet_transport` | `event` | `emit_event` | 停留点释放且负载低于恢复阈值时发出容量恢复事件。 |
| `start_pallet_transport` | `pallet_transport` | `event` | `start_behavior` | 启动左侧托盘进入第一段主传送带。 |
| `transfer_pallet_to_sorting_position` | `pallet_transport` | `event` | `start_behavior` | 第一段主线释放后将托盘送到第二段机械臂分拣位。 |
| `idle_robot_requests_workpiece` | `parallel_robot_sorting` | `event` | `emit_event` | 托盘到位后空闲机械臂请求共享工件池。 |
| `claimed_workpiece_starts_pick` | `parallel_robot_sorting` | `event` | `start_behavior` | claim 成功后机械臂抓取工件并放到策略选中的出料传送带。 |
...
| `target_conveyor_selection` | `load_balancing` | {"candidates": ["main_conveyor_1", "main_conveyor_2", "upper_out_conveyor_1", "lower_out_conveyor_1"], "prefer": "lowest_current_load_not_blocked"} |

### 1.6 期望完成条件 `completion_conditions`

| condition_id | 条件表达 | 依赖状态 |
|---|---|---|
| completion_01 | `workpiece_pool.remaining_parts.empty == true` | `state_model` / RuntimeSnapshot |
| completion_02 | `active_actions.empty == true` | `state_model` / RuntimeSnapshot |
| completion_03 | `all conveyor_loads current_load == 0` | `state_model` / RuntimeSnapshot |
