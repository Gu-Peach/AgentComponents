...
> 版本：v0.3
> 日期：2026-09-06
> 目标：以 `DeviceSpec + SceneDocument -> TopologyGraph -> SceneBehaviorGraph -> RuntimeSnapshot` 作为 Agent 行为建模与运行时执行链路。
...
场景层行为建模
  SceneDocument 定义场景事实；TopologyGraph 将物理边、工艺边和信号边编译为可查询的派生拓扑索引。

行为层运行建模
  SceneBehaviorGraph 定义该场景在用户目标下如何通过事件、状态、策略和行为规则运行。
```
...
| 文档 | 内容 |
|---|---|
| [`device_data_structure.md`](device_data_structure.md) | 当前 v0.3 基线：`DeviceSpec + SceneDocument -> TopologyGraph -> SceneBehaviorGraph -> RuntimeSnapshot`。 |
...
| [`5.TopologyGraph/`](5.TopologyGraph/) | 派生拓扑图规范，用于校验、调度、Agent 摘要和 Runtime 查询。 |
...
| `5.TopologyGraph` | 派生拓扑图 | 派生索引 | 由 DeviceSpec 与 SceneDocument 编译得到，不作为手工事实源。 |
...
TopologyGraph(DeviceSpec + SceneDocument)
  -> 编译 physical_graph / process_graph / signal_graph / transport_graph
...
```text
TopologyGraph 是派生索引，不替代 SceneDocument 三类边事实；
...
- `TopologyGraph` 是从设备能力和场景事实编译得到的派生图索引，用于可达性、信号传播、拓扑摘要和一致性校验。
...
- 如果两边冲突，以 `business/` 中的当前 v0.3 基线为上层依据，再更新 `docs/backend/` 的技术设计。
