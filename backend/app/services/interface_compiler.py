...
class InterfaceCompiler:
    """Compile process-level wiring into physical and signal wiring.
    ...
    backend: process edges remain the source intent, while physical/signal edges
    are generated facts that can be inspected and persisted.
    """
    ...
    def compile(self, scene_doc: dict[str, Any], specs: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        for process_edge in scene_doc.get("process_edges", []):
                ...
                warnings.append(
                    {
                        ...
                        "message": "Process edge cannot compile to physical edge without both process-port bindings.",
                        "process_edge_id": process_edge.get("edge_id"),
                    }
                )
                continue

            source_physical = f"{source_instance['instance_id']}.{source_binding.get('physical_interface')}"
            target_physical = f"{target_instance['instance_id']}.{target_binding.get('physical_interface')}"
            ...
            source_signal_ids = source_binding.get("signal_ports", [])
            target_signal_ids = target_binding.get("signal_ports", [])
